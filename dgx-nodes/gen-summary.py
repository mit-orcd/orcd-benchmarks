...
tables = subprocess.check_output(["python3", "analyze-nccl.py"]).decode()
...
body = """# NCCL benchmark summary -- DGX H100 nodes (node180[0,1], node270[0,1], node280[0,1])

Generated %s by gen-summary.py (analyze-nccl.py over out-1node/, out-2node/).
...
%s
%s
""" % (datetime.datetime.now().strftime("%Y-%m-%d %H:%M"), tables, ceilings)
