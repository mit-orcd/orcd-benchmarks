...
- *IOMMU / IOTLB pressure.* Both clusters boot the identical `iommu=pt intel_iommu=on` with the same 540 groups, and the Ubuntu pair reaches full GPUDirect line rate in both directions *under that setting* (395.5 Gb/s reading from GPU). Whatever separates the clusters, it is not the IOMMU being on. This retires what was suspect #1 in `../b200-nodes/notes.md`.
...
- *A bulk bandwidth cap explaining the small-message gap.* `sendrecv` moves its 1 MiB as one chunk over the same NIC and GPU-memory path and is identical on both clusters, and at line rate at large sizes. A degraded bulk path would slow it too.

**The shape of the per-operation gap constrains the explanation.** The cost is paid **per network operation and per synchronisation**, not per byte: a constant overhead is a large fraction of a 1 MiB transfer and negligible in a 16 GiB one, which is exactly the observed decay (2.23x -> 1.02x). A 1 MiB all_gather becomes 8 chunks of 128 KiB across 8 channels plus cross-phase synchronisation; a 1 MiB sendrecv is one chunk. The layers that set that per-operation cost are the candidates:

1. **The InfiniBand stack — MOFED 25.10 (Ubuntu) vs 26.04 (Rocky 8).** The leading candidate. The verbs provider is precisely the layer that sets per-work-request posting cost while leaving bulk streaming untouched, which is the measured shape. Note the counter-intuitive direction: Rocky 8 runs the *newer* stack and is slower per operation, consistent with a regression in the newer provider.
2. **Host CPU cost in NCCL's proxy thread.** That thread posts every RDMA operation on the host CPU, so its cost scales with operation *count* rather than bytes. A `powersave` governor or deep C-states on the Rocky side would produce exactly this signature — small transfers penalised, streaming untouched, and the penalty decaying as messages grow. The Ubuntu nodes run `performance`. This is the cheapest hypothesis to test and the governor on node5500-5502 has not been read.
3. **GPU driver / CUDA pair — 570.211.01 + CUDA 12.9 vs 590.48.01 + CUDA 13.1.** Affects kernel launch and GDR registration cost, both per-operation. Weaker than (1) because it should also touch the sendrecv path somewhat, and that path is clean.
4. **Kernel version — a uniform 6.8 vs a heterogeneous 4.18 / 6.12 pair.** Running EL8's 4.18 under MOFED 26.04 on node5500 is an unusual combination, and the older kernel's RDMA and DMABUF paths differ materially from 6.8. It is not the sole cause — all three Rocky pairs are slow, including the 5501+5502 pair — but it is a live variable that should simply be removed.
...
- **PCIe ACS state on the Broadcom switches.** With IOMMU on, Linux enables ACS on downstream ports, and ACS redirect routes peer-to-peer TLPs up to the root complex instead of straight across the switch. On node5700 that is evidently not costing anything — `nvidia-smi topo -m` reports **PXB** for every GPU<->rail pair, `lspci -t` confirms HCA and GPU under the same switch, and the read is at line rate — so if it costs on Rocky 8, the difference is in BIOS/firmware ACS state or in the PCIe topology itself. node5502 already carries `pci=disable_acs_redir=pci:1000:c030` and was still capped, which suggests that mask missed the switch ports actually in its GPU<->NIC path.
- **PCIe topology.** If a Rocky node reports `NODE`/`SYS` where node5700 reports `PXB`, the GPU and its rail are not under a common switch there, and the peer-to-peer path is through the host bridge — enough on its own to explain the cap.
- **PCIe Relaxed Ordering (BIOS).** An NVIDIA-recommended setting for GPUDirect; disabling it degrades NIC-reads-from-GPU specifically, which matches the read/write asymmetry (147.6 read vs 286.6 write) better than any symmetric explanation.
...
**And for the one place Rocky 8 wins — the large-message `scatter` plateau — the mechanism is not established.** Plausible configuration-level reasons, none confirmed: the newer MOFED 26.04 may stream root-anchored traffic better at 16 GiB; the NCCL build against CUDA 13.1 may pick a different channel count or protocol for root-anchored patterns than the CUDA 12.9 build; or `pci=realloc=off` on the Ubuntu nodes may leave the root GPU's outbound path configured differently from the Rocky default. Settling it needs `NCCL_DEBUG=INFO` on both sides, comparing channel count, protocol (LL / LL128 / Simple) and algorithm — a low-priority thread, since scatter rarely bottlenecks training.
