...
class MambaHybridModelState(DefaultModelState):
    ...
    def postprocess_state(
        ...
        if not isinstance(num_sampled, int):
            ...
            # LOCAL PATCH (b200-kimi). Upstream crashed here at concurrency >= 16 on
            # Kimi-K3 TP8xPP2:
            #     IndexError: index_fill_(): Expected dtype int64 for index.
            # Two defects, both only reachable on the non-spec / int num_sampled path:
            #   1. idx_mapping is not int64, which index_fill_ requires.
            #   2. The comment above this method states idx_mapping may carry -1
            #      sentinels for filtered rows UNDER PIPELINE PARALLELISM, and the
            #      kernel branch skips them explicitly -- but this branch does not.
            #      index_fill_ interprets -1 as "last element", so a sentinel would
            #      silently corrupt another sequence's accepted-token count rather
            #      than being ignored.
            # Cast, then drop the sentinels, matching the kernel branch's semantics.
            _idx = idx_mapping.to(torch.int64)
            _idx = _idx[_idx >= 0]
            if _idx.numel():
                self.num_accepted_tokens_gpu.index_fill_(
                    0, _idx, max(num_sampled, 1)
                )
