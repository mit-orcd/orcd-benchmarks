...
__global__ void mat_mult(double* dev_matrix, double* mat_1, double* mat_2) {
...
void serial_mat_mult(double* ans_mat, double* mat_1, double* mat_2) {
    int i, j, k;
    for (i = 0;i < dim;i++) {
        for(j = 0;j < dim; j++) {
            ans_mat[i * dim + j] = 0.0;
...
int main() {
    double *host_1, *host_2, *host_ans;
    double *dev_1, *dev_2, *dev_ans;
    struct timeval start, end, diff;

    host_1 = (double*)malloc(dim * dim * sizeof(double));
    host_2 = (double*)malloc(dim * dim * sizeof(double));
    host_ans = (double*)malloc(dim * dim * sizeof(double));
    cudaMalloc((void**)&dev_1, dim * dim * sizeof(double));
    cudaMalloc((void**)&dev_2, dim * dim * sizeof(double));
    cudaMalloc((void**)&dev_ans, dim * dim * sizeof(double));

    srand(time(NULL));
    int i, j;
    for(i = 0;i < dim; i ++) {
        for(j = 0; j < dim; j++) {
            host_1[i * dim + j] = ((double)rand() / RAND_MAX) * max_val;
            host_2[i * dim + j] = ((double)rand() / RAND_MAX) * max_val;
        }
    }
    cudaMemcpy(dev_1, host_1, dim * dim * sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_2, host_2, dim * dim * sizeof(double), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(host_ans, dev_ans, dim * dim * sizeof(double), cudaMemcpyDeviceToHost);
